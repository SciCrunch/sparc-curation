# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; orgstrap-block-checksum: c179c12833eda90dbe26a791b64adeb7b3ed14744f12b8414ff28ea232e9d3dd; -*-
...
#+begin_src bash
emacs -eval '(progn (url-copy-file "https://raw.githubusercontent.com/SciCrunch/sparc-curation/master/docs/queries.org" "~/queries.org" t) (find-file "~/queries.org"))'
#+end_src
...
references that provide supporting evidence.
#+header: :var model_id="https://apinatomy.org/uris/models/keast-bladder" :limit 99
#+begin_src cypher
MATCH (start:Ontology {iri: $model_id})
<-[:isDefinedBy]-(external:Class)
...
*** sparc
:PROPERTIES:
:header-args:cypher: :scigraph http://localhost:9000/scigraph
:END:
**** organParts
#+name: desc-organ-parts
Get the parts list for an organ including nerves and blood vessles
#+header: :var id=(identity nil)
...
#+header: :var species-id=(identity nil)
...
#+header: :var artifact-id=(identity nil)
...
#+end_src
**** rootLabels/{root-id}
#+header: :var root-id=(identity nil)
...
#+header: :var species-id=(identity nil)
#+header: :var region-id=(identity nil)
...
#+header: :var start_id=(identity nil)
#+header: :var end_id=(identity nil)
#+header: :var max_depth=(identity nil)
#+header: :var relationship=(identity nil)
...
*** apinat
**** bundles/{start-id}
#+header: :var start-id=(identity nil)
...
WHERE
{
  ...
  ?link apinatomy:fasciculatesIn ?layer_or_end .
...
#+header: :var process-id=(identity nil)
...
where {
  ...
  ?link ( apinatomy:fasciculatesIn | apinatomy:layerIn | apinatomy:internalIn )+ ?region .
...
#+call: neru-simplified(path="./images/neru-simplified-ntk-2.svg", json-string=python-neru-helper(result=cypher-neru-helper(neupop_id="ilxtr:neuron-type-keast-2")))
...
#+call: neru-simplified(path="./images/neru-simplified-ntk-5.svg", json-string=python-neru-helper(result=cypher-neru-helper(neupop_id="ilxtr:neuron-type-keast-5")))
...
UBERON:0005363 l: "inferior vagus X ganglion" .
...
#+header: :var n-sparql-apinat-models=n-sparql-apinat-models()
...
(let ((models (caar n-sparql-apinat-models))
...
# FIXME owl:Ontology vs elements:Graph
...
(defvar orgstrap-do-config-emacs t "Run code that modifies the Emacs configuration.")
...
(setq-local
 ...
 (lambda (lang _body)
   (not (or (member lang '("cypher" "sparql"))
            (eq (intern (orgstrap-get-named-src-block-checksum "nonl"))
                '6f941ab9e081045c990e9ae4d7dee4604b20724db41883e137ff925f6a9a7875)))))
...
(let ((ghost "https://raw.githubusercontent.com/tgbugs/orgstrap/"))
  (unless (featurep 'ow)
    (reval 'sha256 'a0c21b230565758c0e246c87a2c73289904490aae9c790d209324dd690dcf466
           ;; "~/git/orgstrap/ow.el"
           (concat ghost "efd9ffcdc2bdc91162b307ff261858f60180e638" "/ow.el")))
...
(when enable-local-eval ; FIXME HACK how to know when this was called
  ...
  (when orgstrap-do-config-emacs
    (ow--rainy-day)
    (add-hook 'emacs-lisp-mode-hook #'rainbow-delimiters-mode)
    (ow-enable-config-familiar-1)))
...
#+name: nonl
#+begin_src elisp :exports none :results none :var paragraph=""
