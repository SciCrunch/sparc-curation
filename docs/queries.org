# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; orgstrap-block-checksum: e42e20c1ed711c5f5161f9b531e2e10c1f6e383ab9afb7b0f43146601e96e3bd; -*-
...
#+RESULTS: cypher-neru-3
[[file:./images/neru-3.svg]]

#+name: cypher-neru-helper
#+begin_src cypher :noweb yes :results drawer :limit 99999999
<<cypher-neru-3>>
#+end_src
...
def query(neupop_id):
    resp = requests.get('http://sparc-data.scicrunch.io:9000/scigraph/dynamic/'
                        f'demos/apinat/neru-3/{neupop_id}.json?limit=9999999')
...
#+begin_src elisp :results none
(ow-confirm-once "neru-simplified")
#+end_src

#+header: :var json-string=python-process-neru(result=cypher-neru-helper(neupop_id="ilxtr:neuron-type-keast-6"))
...
*** neur populations projecting to
...
*** neur populations with axons in
...
*** neur populations dendriting to
...
*** neur populations with dendrites in
...
*** neur populations with processes in
...
(defun ow-confirm-once (block-name &optional universal-argument) ; TODO move into orgware for real
