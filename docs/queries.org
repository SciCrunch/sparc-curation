...
where {
  ...
  ?link apinatomy:fasciculatesIn ?region .
  ...
  optional { ?seed apinatomy:seedIn ?neru . } # this sort of works but misses cases where a dynamic group was created and there is no matching seed

}
order by str(?external)
#+end_src
...
| UBERON:0001894 | Diencephalon | keast-bladder     | 15 16 17 18 |
