...
where
{
  ...
  MINUS {  # to get minus to work it needs to share a variable with the core query
    ?dataset a sparc:Dataset ;
    TEMP:hasExperimentalApproach ?approach .
    ?dataset TEMP:hasProtocol ?protocol2 .
...
[[pio.html:29990]] useless, is primarily a sparc curated protocol and just have RRID:Missing instead of identifying it as a slide scanner :/
[[pio.html:22831]] a perfect example of the failure due to inconsistency in how inputs were tagged and are missing a type
...
def raw_adj_to_nst(raw_adj):
    ...
    adj = sorted(set(tuple(orders.rl(*(a if a is None else OntId(a).u for a in rl)) for rl in rls) for rls in raw_adj))
