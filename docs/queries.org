...
#+begin_src sparql :noweb no-export
<<sparql-complex-protocols-1>>
<<sparql-complex-protocols-2>>
#+end_src
...
#+name: sparql-complex-protocols-1
#+begin_src sparql
select distinct
?protocol ?anno_count
where {
...
#+end_src
#+name: sparql-complex-protocols-2
#+begin_src sparql
...
#+begin_src python :noweb yes :var input=(orgstrap--with-block "ttl-organs" body) :results code :wrap src ttl
