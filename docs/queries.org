# -*- org-adapt-indentation: nil; org-edit-src-content-indentation: 0; orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 94d9c49407a4a1e4b3044aa2d33e74026277cb56a6283369cc4ca2f6a90a3d6e; -*-
...
#(str(?raw_in) as ?ris)
where {
  values ?val_asp {asp:magnification} # XXX spelling fix is on the way

  ?protocol a sparc:Protocol ; ?sigh ?ast_asp ; TEMP:protocolInvolvesInput ?ast_in_sp .
  ?ast_in_sp rdf:type protcur:input ; TEMP:hasValue ?species .
  #?species rdfs:subClassOf+ NCBITaxon:1 .
  ?species rdfs:subClassOf+NCBITaxon:33208 . # significantly restrict the search space to only those with nervous systems
  ?species rdfs:label ?sl .
  #filter contains(str(iri(?species)), "NCBITaxon")

  ?ast_asp a protcur:aspect ;
  TEMPRAW:hasValue ?val_raw_asp ;
  TEMP:hasValue ?val_asp ;
  TEMP:protcurChildren ?s .
  ?s a ?type .
  ?s TEMP:hasValue ?bn .
  ?bn ?p unt:fold .
  ?bn rdf:value ?value .
  ?bn TEMP:hasUnit ?unit .

  ?protocol ?pl ?ast_ebb .
  ?ast_ebb TEMP:protcurChildren* ?bn1 .
  ?bn1 TEMP:hasValue ?region . # UBERON:0005363 .  # nodose ganglion
  # ?protocol TEMPRAW:protocolInvolvesInput ?raw_in .  # slow

  values ?nerves_and_ganglia {
    #UBERON:0010313  # neural crest-derived structure better than anatomical entity  # XXX not needed for this particular example
    UBERON:0000122  # neuron projection bundle complex and not covered by ncds
    UBERON:0000045  # ganglion also because it does not classify under the other two
  }
  #filter regex(str(iri(?region)), "(UBERON|ILX):")
  #?region (rdfs:subClassOf|TEMP:partOf)* UBERON:0000010 .
  #?region (rdfs:subClassOf|TEMP:partOf)+ UBERON:0001016 .  # XXX this has to be inserted otherwise it is far too slow
  ?region rdfs:subClassOf+ ?nerves_and_ganglia .  # have to go * for ganglion? (actually no, it is almost certainly a dupe of nodose)
  #?region rdfs:subClassOf+ UBERON:0001062 .  # XXX this is a not-so-great alternative
...
} order by ?region_l ?dataset ?protocol limit 100
#+end_src
...
def apinat_to_raw_adj(blob):
    ...
    raw_adj = [((r1, l1), (r2, l2)) for  # region first since neurondm.orders.rl.__init__(self, region, layer=None)
               ...
               # FIXME no projections case ... do we link to self for soma?
               in cpairs_ok
               # FIXME have to exclude cycles like this for now because neurondm.orders.adj_to_nst breaks if there are cycles like this
               if (r1, l1) != (r2, l2)]
    return raw_adj
