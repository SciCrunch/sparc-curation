...
# FIXME because archives are made in place they appear on the server before they are done
#+begin_src bash
...
pushd ~/.local/share/sparcur/export/summary/618e8dd9-f8d2-4dc4-9abb-c6aaab2e78a0

# tar.xz anything that doesn't have it (i.e. usually just the most recent)
# FIXME TODO an actual archival approach needs a temporary manifest of sha256 checksums, we then sha256 the archive,
# decompress it and make sure the contents checksums match then we know that the archive checksum is sufficient
echo Archiving existing releases.
find -maxdepth 1 -mindepth 1 -type d -not -exec test -f '{}.tar.xz' \; -print | \
xargs -P4 -r -I{} sh -c 'XZ_OPT=-e9 tar -cvJf "{}.tar.xz" {} && { xz -t "{}.tar.xz" || { echo bad archive for {}; rm "{}.tar.xz"; }; }'

# remove all but the last 2 (ie the one we just created and the one before it) make sure we only remove folders that already have an archive
# XXX WARNING if the archiving step above fails OR THIS STEP IS RUN BEFORE THE PREVIOUS ONE FINISHES and a malformed or INCOMPLETE archive is
# present then this will delete the only good copy ...
echo Removing old archived releases.
find -maxdepth 1 -mindepth 1 -type d -exec test -f '{}.tar.xz' \; -print | sort -u | head -n -2 | xargs rm -r
popd
...
TODO Implement =sparcur.simple.compact_snapshot= to relink older
...
*** in place
*This is deprecated in favor of archiving releases immediately after they are compiled.*
