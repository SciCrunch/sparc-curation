...
# IF YOU ARE HERE AFTER RUNING =scimax ~/setup.org= click [[#per-user-setup][this link]]
...
*** Bootstrapping [[./setup.org][this =setup.org= file]]
    ...
    *Read all the text between blocks. It will tell you what to do next.*
    ...
    #+BEGIN_SRC bash :exports code :eval never
      ...
      #--eval '(goto-char (org-find-exact-headline-in-buffer "Per user setup *NEXT HERE*"))' \
      #--eval "(add-hook 'emacs-startup-hook (lambda () (recenter-top-bottom 0)))"
    #+END_SRC
...
*** Per user setup
    ...
    and typing =C-c C-c= (control c control c).
