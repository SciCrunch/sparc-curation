...
#+BEGIN_SRC bash :exports code :eval never
      ...
      mkdir ~/files
      source .profile
      #git clone https://github.com/SciCrunch/sparc-curation.git ~/git/sparc-curation  # FIXME LOL WINDOWS
      # never clone to a target with ~ in it on windows
      # copy and paste from this block into powershell will result
      # in the hilarious problem where there is a literal ~ folder
      # created, thus, use pushd and popd instead which work as expected
      pushd ~/git
