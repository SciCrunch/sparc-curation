...
* Introduction
  ...
  The development environment created by running this was originally developed on Gentoo, and is portable to other distributions with a few tweaks.
* Setup
** One shot
...
*** Bootstrapping [[./setup.org][this =setup.org= file]]
    ...
    *All the code blocks in this Bootstrapping section need to be pasted into a shell where you are logged in as your user*
    #+NAME: setup-folders
    ...
    =scimax= will launch emacs an install a number of packages. It is normal to see errors
    during this step. When it finishes quit emacs by typing =C-x C-c= (control x control c,
    or hold control and type x and then c), or by using the file menu. After running the next
    command you should have a version of this file open locally. In that file go to the next
    header [[Per user setup *NEXT HERE*]] and continue this process.
...
*** Per user setup *NEXT HERE*
    ...
    *and you can move on to the [[Configuration files]] section.*
...
*** Configuration files
    ...
    You can move your [[file:${HOME}/secrets.yaml]] to live where ever you want, but
    you will need to update the =secrets_file= entry in [[file:${HOME}/devconfig.yaml]].
...
*** Accounts and API access
    Create accounts, obtain various API keys. The notation =(-> key1 key2 key3)=
    indicates a path in your [[file:${HOME}/secrets.yaml]] file. After you finish
    this section you can jump to [[Get data][getting data]]!.
...
***** protocols.io
      ...
      Use the information there to fill in a json file with the structure below.
