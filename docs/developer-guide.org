...
There are also two other sets of blocks in [[./apinatomy.org::#npo-identifiers]]
for apinatomy-neuron-populations.py and apinat-pops-more.py that require manual sync.
***** ApiNATOMY
#+begin_src bash
...
from sparcur.simple.fetch_annotations import (
    from_group_name_cached_annos, from_user_name_group_name_h)
...
def main():
    ...
    html_annos = from_group_name_cached_annos('fixed-sparc-curation')
    html_bannos = [ptcdoc.Annotation(a) for a in html_annos]
    html_pool = ptcdoc.Pool(html_bannos)

    h_p_f = from_user_name_group_name_h('protbot', 'fixed-sparc-curation')
