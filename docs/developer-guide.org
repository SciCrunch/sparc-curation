...
#+name: racket-graph
#+header: :wrap "src dot :file ./images/pipeline-fetch-dag.png :cmdline -Kdot -Tpng :exports results :cache yes"
...
#+RESULTS[e29666f9fbc8046ebab291bb30916242ab20cfeb]: racket-graph
#+begin_src dot :file ./images/pipeline-fetch-dag.png :cmdline -Kdot -Tpng :exports results :cache yes
digraph G {
...
[[file:./images/pipeline-fetch-dag.png]]
...
(define-syntax (graph-notation stx)
  (syntax-parse stx
    #:datum-literals (->)
    [(_ (~seq left -> right) ...)
     #'(quote ((left right) ...))]))
