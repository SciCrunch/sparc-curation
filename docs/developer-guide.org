...
# curation export
# XXX see setup.org for most recent workflow for batch

# NPO
python -m neurondm.build release
# super manual curation, identifier sync, review, merge, commit, push, etc.
# TODO there are also two other sets of blocks in apinatomy.org
# apinatomy-neuron-populations.py and apinat-pops-more.py that
# require manual sync right now

# apinatomy.org
~/git/sparc-curation/docs/apinatomy.org --all
# deploy to remote
# apinat-build --deploy  # XXX manual
# apinat-build --tangle  # XXX OLD
# source ~/git/sparc-curation/bin/apinat-functions.sh  # XXX OLD
# for new apinatomy models also update ~/git/sparc-curation/resources/scigraph/sparc-data.ttl

# scigraph/README.org
sh ~/git/pyontutils/nifstd/scigraph/README.org --tangle
# maybe update sparc community terms
# sh ~/git/pyontutils/nifstd/scigraph/README.org --sct  # XXX manual
# currently run sparc-community-terms-update, review, merge, and push to NIF-Ontology
...
### annoying apinatomy npo sync

# XXX manual
# right you now have to go through all the steps above at least up to the scigraph load
# and then run [[queries.org::*generate npo representation of apinatomy population location phenotypes]]
# to generate apinatomy-locations.ttl delete the junk at the bottom of the file
# save, commit, push, merge, push and THEN you have to run the whole export process AGAIN
# adding a good hour or so to the release process
...
# run the docker build blocks in source.org for the sckan release that
# do all the copying of the build state to the requisite locations
# order is bottom to top (maybe us ob-lob)
# &musl-build-sckan-base
# &musl-build-sckan-services

### testing

# TODO automate
# create the new docker container volume for sckan data
# run it with the latest version of tgbugs/musl:kg-release-user
# run all the examples etc.
...
#from sparcur.utils import fromJson, register_all_types  # FIXME this should go in sparcron
...
def main(project_id=None, export_path=None, time_now=None,
    ...
    ttl_all, ttl_pub = from_snapshot_path_summary_ttl(
    ...
    npath = pexpath / maxdt  # reminder that maxdt is in zulu
    if not npath.exists():
...
def from_dataset_path_metadata_file_paths(dataset_path):
    # FIXME TODO shouldn't this be ALL xlsx files?
    return rglob(dataset_path, '*.xlsx')
