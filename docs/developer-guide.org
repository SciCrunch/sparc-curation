...
def from_path_cleaned_object(path, for_template=False, template=_dmpt):
    t = dat.Tabular(path)
    sheet, wb, sparse_empty_rows = t._openpyxl_fixes(for_template=for_template, template=template)
    return wb
...
def from_file_paths_objects(paths, for_template=False, known_templates=_dmpt):
    ...
    for path in paths:
        if template is not None and path.stem not in template:
            yield None
            continue

        if path.suffix == '.xlsx':
            cleaned = from_path_cleaned_object(path, for_template, template)
            yield cleaned
        else:
            yield None
