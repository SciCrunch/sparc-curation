...
See [[./workflows.org::#export-v6]] for the most recent workflow for batch release.
***** SPARC protcur
#+begin_src bash
...
***** NIF-Ontology merge into dev branch
The release always builds from the dev branch so any changes you want
in a release should be merged from the various project branches into
dev. Add additional branches as needed.
#+begin_src bash
...
#+begin_src bash :tangle ../bin/build-sckan-release
set -e
...
~/git/sparc-curation/docs/release.org build --sckan --no-blaze --no-load --no-annos
# XXX remove --no-annos if ~/.ssh/config has cassava-sparc
...
# release.org
~/git/sparc-curation/docs/release.org build --sckan --no-blaze --resume --no-annos
# XXX remove --no-annos if ~/.ssh/config has cassava-sparc
...
_sckandz="$(ls -d /tmp/build/docker-sckan-data-*Z.tar.gz | sort -u | tail -n 1)"
...
def main(id=None, **kwargs):
    ...
    if ik('git_repos'):
        ...
        ind = 8 if sys.platform == 'win32' else 7
        packs = list(d.discover())
        alt = [p for p in [aug.RepoPath(d.origin.url[ind:]).resolve() for d in packs if hasattr(d, 'origin') and d.origin] if p.working_dir]
        rps = [p.parent for p in [aug.RepoPath(d._path).resolve() for d in packs] if p.working_dir]
        srps = set(alt) | set(rps)
        setups = [p for p in [p / 'setup.py' for p in srps] if p.exists()]
        wds = sorted(set([p.working_dir for p in srps]))
