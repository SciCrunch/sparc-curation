...
function sparcur-get-all-remote-data () {
    ...
    iso8601millis="+%FT%T,%6N%:z"  # FIXME do we _really_ need millis!? yemaybe? concurrent startups?
    local TIME_START=$(date ${iso8601millis} || gdate ${iso8601millis})
    local TIME_START_SAFE=$(echo ${TIME_START} | sed 's/[:-]//g')
    local TIME_START_FRIENDLY=$(echo ${TIME_START} | sed 's/://g')
    mv "$(mktemp --directory sparcur-all-XXXXXX)" "${TIME_START_FRIENDLY}" || \
        { CODE=$?; echo 'mv failed'; return $CODE; }
    local PARENT_PATH=${TIME_START_FRIENDLY}
