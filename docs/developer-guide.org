...
(define g (dag-notation
           ...
           pipeline-end -> pipeline-extras -> sparc-ds -> pipeline-start -> description -> fetch-all
                           ...
                           pipeline-extras -> extras-added
                           pipeline-extras -> lifters
                           pipeline-extras -> contributors -> affiliations #;lifters -> affiliations-sheet -> network
                                              contributors -> member #;lifters -> blackfynn-api -> network
                                              contributors -> description
                           pipeline-extras -> meta -> organ-term #;lifters -> organs-sheet -> network
                                              meta -> modality #;lifters -> organs-sheet
                                              meta -> techniques #;lifters -> organs-sheet
                                              meta -> protocol-uris #;lifters -> organs-sheet
                                              meta -> award-manual #;lifters -> organs-sheet
                                              meta -> dataset-doi -> blackfynn-api ; path.cache.remote.bfobject.doi ; what a hack :x 
                                              ; FIXME TODO this is how dataset-doi _should_ work
                                              ; uncomment when it _actually_ works like this
                                              ; meta -> dataset-doi -> cache-remote-metadata -> fetch-all
                                              meta -> award-organ -> scraped-award-organ
                                                      award-organ -> submission-normalized
           ??? -> overview-sheet -> network))
(graphviz g)
#+end_src

#+name: graph-validate-all-dot
#+RESULTS[90c92629fb562228be77ac598ef3bc30470a5afc]: graph-validate-all
#+begin_src dot :file ./images/graph-validate-all.png :cmdline -Kdot -Tpng :exports results :cache yes
digraph G {
	node0 [label="description"];
	node1 [label="extras-added"];
	node2 [label="dataset-doi"];
	node3 [label="blackfynn-api"];
	node4 [label="dataset-metadata"];
	node5 [label="meta"];
	node6 [label="samples"];
	node7 [label="subjects"];
	node8 [label="award-manual"];
	node9 [label="submission-normalized"];
	node10 [label="organs-sheet"];
	node11 [label="scraped-award-organ"];
	node12 [label="member"];
	node13 [label="lifters"];
	node14 [label="award-organ"];
	node15 [label="network"];
	node16 [label="submission"];
	node17 [label="fetch-all"];
	node18 [label="manifest"];
	node19 [label="techniques"];
	node20 [label="overview-sheet"];
	node21 [label="pipeline-end"];
	node22 [label="pipeline-start"];
	node23 [label="protocol-uris"];
	node24 [label="pipeline-extras"];
	node25 [label="dataset-structure"];
	node26 [label="contributors"];
	node27 [label="organ-term"];
	node28 [label="affiliations"];
	node29 [label="affiliations-sheet"];
	node30 [label="???"];
	node31 [label="sparc-ds"];
	node32 [label="modality"];
	subgraph U {
		edge [dir=none];
	}
	subgraph D {
...
#+RESULTS[0fda54b4f1f55a0506b6a6fc085ce04622515764]: graph-validate-all-dot
