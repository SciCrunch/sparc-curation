...
def from_path_dataset_file_structure_all(project_path, *args, paths=None, time_now=None):
...
#+begin_src python :tangle ../sparcur/simple/extract.py :mkdirp yes
...
def dataset_raw(*objects, object_type_key_map=otkm):
    ...
    for obj in objects:
        log.debug(obj)
        try:
            # FIXME isinstance probably needed due to Paths ... :/
            try:
                key = object_type_key_map[type(obj)]
                data.update({key: obj.data})  # FIXME this seems too naievely simple
            except KeyError:
                # AAAAAAAAAAAAAAAAAAAAAA this is so stupid inhomogenous and busted
                # decide whether the objects themselves determine where they sit
                # or whether this method does this is now WAY too complicated
                # to be easily understood and all sorts of nonuniformity in the
                # underlying processes is leaking through ugh 1 vs many issues :/
...
def from_path_dataset_raw(dataset_path):
    ...
    try:
        ...
        logd.critical(e)
        return early_failure(dataset_path, e)
    ...
    return dataset_raw(ddo, *comb_dataset(dataset_path, dtsv))
...
# NOTE we capture version as early as possible in the process, yes we
...
class mmf:  # sigh patterns are stupid, move this to elsewhere so it doesn't taint everything
    def __init__(self, data):
        self.data = data
otkm[mmf] = 'manifest_file'  # FIXME this defeats the purpose of otkm in the first place!!!!
# it obsures what is going on and makes it harder to understand what is supposed to be documentation
...
def merge_manifests(vs):
    ...
    return mmf([m for v in vs for m in v.data[k]])
    #return type('mmmmmmm', tuple(), dict(data = {k: [m for v in vs for m in v.data[k]]}))


def from_path_dataset_path_structure(path, version):
    return dat.DatasetStructure(path)


class sigh:  # the things we have to do to avoid special cases >_<
    def __init__(self, data):
        self.data = data


otkm[sigh] = 'rmeta'  # FIXME this defeats the purpose of otkm in the first place!!!!
def from_path_remote_metadata(path, _version):
    # version is not used for this at the moment
    return sigh(BlackfynnDatasetData(path.cache).fromCache())


from_path_dataset_description = object_from_find_path('dataset_description', description, onfail=exc.MissingFileError)
...
from_path_manifests           = multiple(object_from_find_path('manifest',   manifest, 'rglob'),
                                         merge_manifests)
    ...
    def combinator(*args, **kwargs):
        for f in functions:
            ...
            if isinstance(obj, GeneratorType):
                # FIXME this is really bad because you don't know what key
                # they are expected to show up under and you can just pair them
                # after the fact, you have to know at the same time >_< so the
                # objects themselves have to know where they are supposed to fit
                # in the dict, or dataset has to keep a mapping between the object
                # types and the key they go under ... this last option seems better
                yield from obj
...
# FIXME this is woefully incomplete, it only gets the raw data
# it doesn't deal with any transformations at all
...
#+begin_src python :tangle ../sparcur/simple/transform.py :mkdirp yes
def main(path=Path.cwd(), **kwargs):
