...
def get_neuron(u):
    ...
    for group in ocg[u:apinat.seedIn:]:
        for ot in ocg[group:apinat.ontologyTerms:]:
            return ot


seeds = {e: neuron for e in somas if (neuron := get_neuron(e))}  # FIXME right now we always seed on soma, but that is not required
...
# OntId('NLX:154731').u  # soma i think? not in scigraph
}
...
[(seeds[n[0]] if n[0] in seeds else None, n) for n in l_split_nst]
