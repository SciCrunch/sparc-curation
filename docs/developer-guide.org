...
function sparcur-get-all-remote-data () {
    ...
    mv "$(mktemp --directory sparcur-all-XXXXXX)" "${TIME_START}" || { CODE=$?; echo 'mv failed'; return $CODE; }
...
def sparse_materialize(path, sparse_limit=None):
    """ given a path mark it as sparse if it is a dataset beyond the sparse limit """
