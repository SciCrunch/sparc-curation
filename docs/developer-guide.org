...
Options:
    ...
    --for-template                  run additional regularization steps when generating templates
...
def from_path_cleaned_object(path, for_template=False):
    t = dat.Tabular(path)
    sheet, wb, sparse_empty_rows = t._openpyxl_fixes(for_template=for_template)
    return wb


def from_file_paths_objects(paths, for_template=False):
    for path in paths:
        if path.suffix == '.xlsx':
            cleaned = from_path_cleaned_object(path, for_template)
            yield cleaned
        else:
            yield None


def from_dataset_path_cleaned_files(dataset_path, cleaned_output_path, for_template=False):
    "NOTE this actually does the cleaning"

    paths = from_dataset_path_metadata_file_paths(dataset_path)
    for path, obj in zip(paths, from_file_paths_objects(paths, for_template)):
        if obj is not None:
...
def main(path=Path.cwd(), id=None, time_now=None,
    ...
    from_dataset_path_cleaned_files(path, cleaned_output_path, for_template)
