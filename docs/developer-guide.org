...
sparcur-do-dataset https://api.pennsieve.io/datasets/N:dataset:${UUID}
sparcur-do-dataset https://app.pennsieve.io/N:organization:618e8dd9-f8d2-4dc4-9abb-c6aaab2e78a0/datasets/N:dataset:${UUID}
#+end_src
Desired behavior.
...
*** _*Fetch and export*_
This is a quick and dirty version that should just do the right thing
given only the dataset id as an input.

Usage =sparcur-fexport N:dataset:totally-not-a-uuid=. The id provided
may be any of the variants, url, curie, api, human, uuid, etc.
#+begin_src bash
# DSID="set it yourself"
function sparcur-fexport () {
    local ID=${1:-${DSID}}
    local UUID
    UUID=$(python -m sparcur.simple.utils --dataset-id "${ID}")
    python -m sparcur.simple.retrieve --dataset-id "${ID}" &&  # TODO --symlink-objects-to
    pushd "$(realpath "${UUID}/dataset")" &&
    ...
    spc find --name '*.xlsx' \
             ...
             --fetch &&
    spc export
    popd # do it yourself because we might need to explore?
}
#+end_src

# FIXME TODO can we wrap this in a really light racket gui or something?
# so that they can just click on buttons to run stuff and see the output?
# it would be a lot easier than trying to route it through a python gui
# or deal with the cli
# https://github.com/telephil/racket-json-view
# the thing is that we can't do this all remotely because they need to
...
def main(parent_path=None,
         ...
         project_id_auth_var='pennsieve-organization',  # FIXME move default to clifun
         symlink_objects_to=None,
         id=None,
         dataset_id=None,
         ,**kwargs):
