# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1-0; orgstrap-block-checksum: 0dee7caf276405a12ae6f60949deef58924ed0faf5e9677a1df624e13757959f; -*-
...
(defun step-fetch-latest-release-zip (&optional build-dir)
  "fetch the previous release and pull what we need"
  ;; annotations aren't changing right now
  (let* ((url "https://api.github.com/repos/SciCrunch/NIF-Ontology/releases")
         (json (reval-url->json url))
         (latest-sckan
          (car
           (sort
            (cl-loop
             for h across-ref json
             when (string-prefix-p "sckan-" (gethash "tag_name" h))
             collect h)
            (lambda (a b) (string> (gethash "tag_name" a) (gethash "tag_name" b))))))
         (tag-name (gethash "tag_name" latest-sckan))
         (release-asset
          (car
           (cl-loop
            for asset across-ref (gethash "assets" latest-sckan)
            when (and
                  (string-prefix-p "release-" (gethash "name" asset))
                  (string-suffix-p "-sckan.zip" (gethash "name" asset)))
            collect asset)))
         (release-name (gethash "name" release-asset))
         (release-zip-path (expand-file-name release-name (or build-dir (build-dir)))) ; FIXME TODO build
         (release-digest (gethash "digest" release-asset))
         (release-cc (string-split release-digest ":"))
         (release-cypher (intern (car release-cc)))
         (release-checksum (cadr release-cc))
         (release-url (gethash "browser_download_url" release-asset)))
    (securl release-cypher release-checksum release-url release-zip-path)
    release-zip-path))
...
(defun step-annos-from-zip (release-zip-path release-dir &optional do-protcur)
  (let* ((name (file-name-base release-zip-path))
    ...
    (run-command "unzip" "-j" release-zip-path annos-zip-path "-d" (expand-file-name "data" release-dir))
    (when do-protcur
      (run-command "unzip" "-j" release-zip-path protcur-zip-path "-d" (expand-file-name "data" release-dir)))))
  ...
  (let* ((path-queries (concat default-directory "queries.org"))
    ...
    (let* ((default-directory build-dir)
      ...
      (let ((default-directory release-dir))
        ...
        (if no-annos
            (let ((release-zip-path (step-fetch-latest-release-zip build-dir)))
              (step-annos-from-zip release-zip-path release-dir 'do-protcur) ; FIXME do-protcur option
              (step-annotations))
          (step-annotations))
