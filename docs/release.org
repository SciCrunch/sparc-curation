# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--dprp-1\.0; orgstrap-block-checksum: 46e9ba16f6e3dda982c100a032d00f6bea256476d2c808bd077e5423079b771c; -*-
...
(defun get-ontology (iri merged-path)
  (run-command ;; FIXME not clear that this is the best approach here
   ...
   "--output" merged-path)
  (run-command
   (or (executable-find "pypy3") (executable-find "python"))
   "-m"
...
(defun step-load-store (path-br-bin)
  ...
  (let ((exec-path (cons path-br-bin exec-path))
    ...
    ;; 7
    (unless-file-exists-p journal-path
...
(defun step-release (build-dir &optional this-release-dir release-time no-blaze)
  ...
  (let* ((path-queries (concat default-directory "queries.org"))
    ...
    (let* ((default-directory build-dir)
      ...
      (let ((default-directory release-dir))
        ...
        (step-annotations)
        (step-load-store (concat path "bin"))
...
#+header: :var rel_path=(or release-dir)
...
#+header: :var rel_path=(or release-dir) prefixes_path=(or prefixes-path)
      ...
      (let ((build-dir (build-dir)))
        (if resume
            (let ((resume-dir (find-resume-dir build-dir)))
              (if resume-dir
                  (step-release build-dir resume-dir nil no-blaze)
                (message "No build to resume.")))
          (step-release build-dir release-dir nil no-blaze))))))
#+end_src
