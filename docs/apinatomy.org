# -*- mode: org; orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; orgstrap-block-checksum: cbf50ae5be0ecc8a1f980756ecde8143f1d509a9912997f7f1aef27c88a4e193; -*-
...
(defun sentinel (process message)
  (when (memq (process-status process) '(exit signal))
    (message "completed: %S" process)
    (setq apinat-process-results
          (cons (list (process-exit-status process)
...
(defun-local update-all-models-async ()
  (let ((model-ids (all-models)))
    ...
    (dolist (model-id model-ids)
      ;; FIXME TODO wait for all processes to finish or something
