# -*- orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; orgstrap-block-checksum: 03bbd6867a3e7164ad35c6141ca4a708e19639661a830c63cf3119cd4ffb5ab8; -*-
...
(let ((args-left command-line-args-left))
  ...
  (when (and noninteractive ; have to use this since --batch is stripped
             ...
             (string= (cadr args-left) "--model-id"))
    (let ((model-id (intern (caddr args-left))))
...
-l ~/git/orgstrap/orgstrap-batch-helper.el \
--visit ~/git/sparc-curation/docs/apinatomy.org \
-- --model-id keast-bladder
#+end_src
