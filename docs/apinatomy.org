# -*- mode: org; orgstrap-cypher: sha256; orgstrap-norm-func-name: orgstrap-norm-func--prp-1\.1; orgstrap-block-checksum: 150ebc3b0d2c2e42ede070a6d47f4b3328c7e8462fcd0abed8fd4861dcfa71f0; -*-
...
# This block works like a super powered shebang line :D! It is used to
# pass the correct arguments to emacs in order to run this file.
#+name: orgstrap-shebang-block
#+begin_src bash :exports none :eval never :results none
function = () { :; }
...
__PARGS=${@}
function posix () {
    test $ZSH_VERSION && { setopt shwordsplit; _IFS=$IFS; unset IFS; } || true
    emacs --quick --batch --load ~/git/orgstrap/orgstrap-batch-helper.el --visit "${__FILE}" -- ${__PARGS}
    test $ZSH_VERSION && { unsetopt shwordsplit; IFS=$_IFS; } || true
}
"posix" > $null
"exit" > $null
$ErrorActionPreference= "Continue"

$org=(Get-Item $MyInvocation.MyCommand.Source).Target
emacs --quick --batch --load ~/git/orgstrap/orgstrap-batch-helper.el --visit $org -- $args
exit
<# open powershell comment
#+end_src
# FIXME $org is a temporary hack until I can work out what is going on with `vc-mode'
...
(defmacro apinat--with-model (model-id &rest body)
  ...
  `(let ((default-directory (expand-file-name (symbol-name ,model-id) apinat-model-repo))
  ...
  (let ((path-xlsx (concat default-directory "/" path-xlsx))
    ...
    (unwind-protect
        (let* ((default-directory path-temp)
               ...
               (_ (run-command apinat-converter-command
                               "-f" "id"
                               "-t" "xlsx"
                               "-t" "json"
                               "-t" "json-flattened"
...
(defun-local all-models ()
  ...
  (let ((default-directory (expand-file-name apinat-model-repo)))
