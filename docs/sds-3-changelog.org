...
- Add =no-transitive= column. TODO maybe =transitive= to force instead? Depends on default.
  Use to prevent an SDS entity id folder from preventing a check on subfolders.
  Default behavior is to warn for nested folders with no entity metadata and no modality.
  # TODO naming
*** COMMENT old thinking
...
- TODO add number_of_performances
- TODO add number_of_sites
...
- Add =license-identifier= field. Controlled to SPDX short identifiers. XXX probably put this in related identifiers?
- Delete
- Change
- Rename
- Fix contributor roles list for ...
